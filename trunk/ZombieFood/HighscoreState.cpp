...
void HighscoreState::enter()
{
	...
	m_GUIEnv->addImage(m_VideoDriver->getTexture("../../media/highscore.jpg"),core::position2d<s32>(0,0)); //2d texture UI

//	m_GUIEnv->addStaticText(L"THIS IS Highscore SCREEN!!", rect<s32>(10,10,260,22), true);
	...
	while( theDevice->run() && (m_bEndState == false) )
	{
		...
		m_VideoDriver->beginScene(true, true, SColor(100,0,100,0));//(a,r,g,b)
